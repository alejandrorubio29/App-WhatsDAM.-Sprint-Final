...
class LoginViewModel(application: Application):
    ...
    public fun Login(username: String, server: String) {
        ...
        viewModelScope.launch(Dispatchers.Main) {
            ...
            val resultat = withContext(Dispatchers.IO) {
                ...
                var resposta = MissatgeRepository.getInstance().Login(username, server)
                Log.d("Debug [LoginViewModel]", "REP: "+resposta.toString())

                // No podem assignar directamen el valor,
                // sino que hem d'utilitzar postValue
                _loginStatus.postValue(resposta) //Dependiente de implementar multihilo?
